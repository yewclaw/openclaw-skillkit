...
async function handleRequest(request, response) {
    try {
        ...
        if (method === "POST" && url.pathname === "/api/review") {
            ...
            const targetDir = requireString(body.targetDir, "targetDir");
            const outputPath = typeof body.outputPath === "string" && body.outputPath.trim() ? body.outputPath : undefined;
            const review = await (0, workflow_1.reviewSkill)(targetDir, outputPath, typeof body.baselineArchivePath === "string" && body.baselineArchivePath.trim()
        ...
        if (method === "POST" && url.pathname === "/api/inspect") {
            ...
            let inspected = typeof body.sourceDir === "string" && body.sourceDir.trim()
                ? await (0, workflow_1.compareArchiveToSource)(archivePath, body.sourceDir, { entryPath })
                : await (0, workflow_1.inspectSkillArchive)(archivePath, { entryPath });
            if (typeof body.baselineArchivePath === "string" && body.baselineArchivePath.trim()) {
                const releaseCompared = await (0, workflow_1.compareArchives)(archivePath, body.baselineArchivePath, { entryPath });
                inspected = {
                    ...inspected,
            ...
            sendJson(response, 200, {
                ...
                releaseDeltaSummary: (0, workflow_1.summarizeReleaseDelta)(inspected),
