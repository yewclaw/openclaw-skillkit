...
export async function lintSkill(skillDir: string): Promise<LintResult> {
  ...
  for (const reference of getReferencedLocalFiles(frontmatterBody)) {
    ...
    if (
      ...
      issues.push(
        createIssue(
          ...
          "SKILL.md",
          `Referenced local file escapes the skill root: ${reference}`,
          {
            category: "references",
    ...
    if (referenceStat.isDirectory()) {
      issues.push(
        createIssue("error", "reference-target-is-directory", "SKILL.md", `Referenced local path is a directory: ${reference}`, {
          category: "references",
  ...
  if (attributes.description) {
    ...
    if (PLACEHOLDER_DESCRIPTION_PATTERNS.some((pattern) => pattern.test(attributes.description))) {
      issues.push(
        createIssue(
          "warning",
          "placeholder-frontmatter-description",
          "SKILL.md",
...
function validateRecommendedSections(sections: Map<string, string>, issues: LintIssue[]): void {
  for (const item of RECOMMENDED_SECTION_HEADINGS) {
    if (!sections.has(item.heading.toLowerCase())) {
      issues.push(
        createIssue(
          "warning",
          `missing-section-${item.heading.toLowerCase()}`,
          "SKILL.md",
...
function getReferencedLocalFiles(markdown: string): string[] {
