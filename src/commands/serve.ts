...
export function getStudioAssets(): { html: string; css: string; js: string } {
  return {
    html: HTML_PAGE,
    css: CSS_PAGE,
    js: JS_PAGE
...
export async function runServe(options: RunServeOptions): Promise<StudioServerHandle> {
...
export async function startStudioServer(options: RunServeOptions): Promise<StudioServerHandle> {
...
async function handleRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
  try {
    ...
    if (method === "GET" && url.pathname === "/api/state") {
      const examples = await listExampleSkills();
      sendJson(response, 200, {
        cwd: process.cwd(),
        templates: Object.keys(TEMPLATE_MODES),
        examples
      });
      return;
    }
    ...
    if (method === "POST" && url.pathname === "/api/init") {
      ...
      sendJson(response, 200, {
        ...
        template,
        resources: [...new Set([...TEMPLATE_MODES[template], ...resources])],
        skillFile: path.resolve(targetDir, "SKILL.md")
      });
      return;
    }
    ...
    if (method === "POST" && url.pathname === "/api/lint") {
      ...
      const result = await lintSkill(resolvedDir);
      ...
      sendJson(response, 200, {
        ...
        nextSteps: buildActionPlan(result, resolvedDir),
    ...
    if (method === "POST" && url.pathname === "/api/pack") {
      ...
      const packed = await packSkill(targetDir, outputPath);
...
function parseTemplate(value: unknown): TemplateMode {
  if (typeof value !== "string" || !(value in TEMPLATE_MODES)) {
    throw new Error(`Template must be one of: ${Object.keys(TEMPLATE_MODES).join(", ")}.`);
  }
