...
type MaintenanceOperationMode =
  | "promote-missing-baseline"
  | "sync-release-baseline"
  | "remove-orphaned-baseline"
  | "remove-artifact";

interface MaintenanceOperation {
  mode: MaintenanceOperationMode;
  label: string;
  sourcePath?: string;
  targetPath: string;
  detail: string;
...
interface MaintenanceResult extends MaintenanceOperation {
  outcome: "pending" | "applied" | "skipped" | "failed";
}

export async function runIndex(indexPath: string, options: RunIndexOptions): Promise<number> {
  const resolvedIndexPath = path.resolve(indexPath);
  const payload = JSON.parse(await readTextFile(resolvedIndexPath)) as unknown;
  ...
  const summary =
    detectedType === "review"
      ? buildReviewSummary(payload as ReviewIndexShape)
      : buildInspectSummary(payload as InspectIndexShape);
  const actionGroups =
    detectedType === "review"
  ...
  const operationGroups =
    detectedType === "review"
      ? buildReviewOperationGroups(payload as ReviewIndexShape)
      : buildInspectOperationGroups(payload as InspectIndexShape);
  ...
  if (options.applyName) {
    ...
    if (!operationGroups.has(selectedOperationGroup.name)) {
      throw new Error(
        `index --apply only supports actionable groups. Use one of: ${[...operationGroups.keys()].join(", ")}.`
      );
    }
...
function selectActionGroup(groups: ActionGroup[], listName: string | undefined): ActionGroup | undefined {
  if (!listName) {
    return undefined;
  }
  ...
  throw new Error(
    `Unknown action group "${listName}". Available groups: ${groups.map((group) => group.name).join(", ")}.`
  );
}

function detectIndexType(payload: unknown): IndexType {
...
function isReviewIndex(payload: unknown): payload is ReviewIndexShape {
  if (!payload || typeof payload !== "object") {
    return false;
  }

  const candidate = payload as Partial<ReviewIndexShape>;
...
function isInspectIndex(payload: unknown): payload is InspectIndexShape {
  if (!payload || typeof payload !== "object") {
    return false;
  }

  const candidate = payload as Partial<InspectIndexShape>;
...
function buildReviewSummary(index: ReviewIndexShape): IndexSummary {
  return {
    headline: "batch review index",
    status:
      index.summary.notReady > 0
        ? "NOT READY"
...
function buildInspectSummary(index: InspectIndexShape): IndexSummary {
  return {
    headline: "batch inspect index",
    status:
      index.summary.releaseChanged > 0 || index.summary.baselineMissing > 0
...
function buildReviewActionGroups(index: ReviewIndexShape): ActionGroup[] {
  return [
    {
      name: "blocked-skills",
...
function buildInspectActionGroups(index: InspectIndexShape): ActionGroup[] {
  return [
    {
      name: "release-changes",
...
function buildReviewCommandGroups(index: ReviewIndexShape): Map<string, string[]> {
  ...
  return new Map<string, string[]>([
    ...
    [
      "blocked-artifacts",
      buildUniqueCommands(
        (index.operationsSummary.blockedSkillArtifacts ?? []).map((artifactPath) =>
          `rm -f ${shellQuote(resolveReviewArtifactPath(index, artifactPath))}`
        )
      )
    ],
    [
      "stale-artifacts",
      buildUniqueCommands(
        (index.operationsSummary.staleArtifacts ?? []).map((artifactPath) =>
          `rm -f ${shellQuote(resolveReviewArtifactPath(index, artifactPath))}`
        )
      )
    ],
...
function buildReviewOperationGroups(index: ReviewIndexShape): Map<string, MaintenanceOperation[]> {
  const skills = index.skills ?? [];
  const byRelativeDir = new Map(skills.map((skill) => [skill.relativeDir, skill] as const));

  return new Map<string, MaintenanceOperation[]>([
    [
      "release-changes",
      buildUniqueOperations(
        index.operationsSummary.skillsWithReleaseChanges.map((relativeDir) => {
          const skill = byRelativeDir.get(relativeDir);
          if (!skill?.archive?.destination || !skill.archive.releaseComparison?.baselineArchivePath) {
            return undefined;
    ...
    [
      "orphaned-baselines",
      buildUniqueOperations(
        (index.baselineSummary?.orphanedArchives ?? []).map((archivePath) => ({
...
function buildInspectOperationGroups(index: InspectIndexShape): Map<string, MaintenanceOperation[]> {
  const archives = index.archives ?? [];
  const byRelativePath = new Map(archives.map((archive) => [archive.relativePath, archive] as const));
...
function buildUniqueOperations(operations: Array<MaintenanceOperation | undefined>): MaintenanceOperation[] {
  const seen = new Set<string>();
  const unique: MaintenanceOperation[] = [];
...
async function applyOperations(operations: MaintenanceOperation[], execute: boolean): Promise<MaintenanceResult[]> {
  const results: MaintenanceResult[] = [];

  for (const operation of operations) {
    if (operation.mode === "remove-orphaned-baseline" || operation.mode === "remove-artifact") {
      const targetExists = await exists(operation.targetPath);
      if (!targetExists) {
    ...
    const sourceExists = await exists(operation.sourcePath);
    if (!sourceExists) {
    ...
    await ensureDir(path.dirname(operation.targetPath));
...
function operationAsResult(
  operation: MaintenanceOperation,
  outcome: MaintenanceResult["outcome"]
): MaintenanceResult {
  return {
    ...operation,
    outcome
...
function formatOperationOutcome(outcome: MaintenanceResult["outcome"]): string {
  switch (outcome) {
    case "applied":
      return "APPLY";
    case "pending":
...
function resolveReviewArtifactPath(index: ReviewIndexShape, artifactPath: string): string {
  if (path.isAbsolute(artifactPath)) {
    return artifactPath;
  }

  return path.join(index.artifactDir, artifactPath);
}
